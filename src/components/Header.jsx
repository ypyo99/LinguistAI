...
export default function Header({ title = "병원 진료 표현 20개", sub = "오늘의 회화 연습", total = 20, progress = 0, streak = 0, onResetProgress, onOpenSettings, onFocusMode, activeTab }) {
  ...
  const silentLogin = useGoogleLogin({
    ...
    onError: (err) => {
      ...
      window.dispatchEvent(new CustomEvent('token_refreshed', { detail: null }));
    },
  });
  ...
  useEffect(() => {
    // 웹 환경에서는 COOP 문제를 vite 설정(unsafe-none)으로 해결했으므로 
    // 백그라운드 silent refresh를 다시 안전하게 사용할 수 있습니다.
    if (user && !Capacitor.isNativePlatform()) {
      registerSilentRefresh(silentRefreshFn);
      startAutoRefresh(silentRefreshFn);
    } else {
      stopAutoRefresh();
  ...
  const headerStyle = {
    backgroundImage: `linear-gradient(135deg, rgba(204, 85, 0, 0.85), rgba(255, 111, 97, 0.85)), url('${LANDMARKS[bgIndex]}')`,
