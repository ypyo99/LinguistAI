...
export default function SetupTab({ apiKey, onSave, ttsApiKey, onSaveTtsKey }) {
  ...
  const handleSaveTts = () => {
    onSaveTtsKey(ttsInput ? ttsInput.trim() : '');
    setTtsSaved(true);
  ...
  return (
    <div className="tab-fade-in flex flex-col gap-4 sm:gap-lg max-w-2xl mx-auto">
      ...
      {/* ── 2. Google Cloud TTS API 키 (고품질 음성용) ── */}
      <section
        className="rounded-xl shadow-sm border border-outline-variant dark:border-outline p-4 sm:p-lg transition-colors duration-200"
        style={{ background: 'var(--surface)' }}
      >
        <h2 className="text-base sm:text-title-md font-semibold mb-2 flex items-center gap-2 text-on-surface dark:text-on-dark-surface">
          <span className="material-symbols-outlined text-xl">record_voice_over</span>
          Google Cloud TTS API 키
          <span className="text-xs font-normal text-on-surface-variant dark:text-on-dark-surface-variant ml-1">(고품질 음성)</span>
        </h2>
        <p className="text-xs mb-4" style={{ color: 'var(--ink-soft)' }}>
          설정하면 영어·한국어 Neural2 고품질 음성으로 재생됩니다. 미설정 시 기기 내장 음성 사용. 월 100만 글자 무료.
        </p>
        <div className="flex flex-col gap-3 sm:gap-md">
          <div className="flex flex-col gap-1 sm:gap-base">
            <div className="flex justify-between items-center">
            ...
            <div className="relative">
              <input
                className="w-full h-10 sm:h-11 px-3 sm:px-md pr-12 rounded-lg border border-outline-variant dark:border-outline bg-surface-container-lowest dark:bg-dark-bg text-on-surface dark:text-on-dark-surface input-focus-ring placeholder:text-outline-variant dark:placeholder:text-on-dark-surface-variant transition-colors duration-200 text-sm font-mono"
                id="ttsApiKey"
                placeholder="AIzaSy... 형식의 GCP API 키"
                type={ttsShow ? 'text' : 'password'}
                value={ttsInput}
                onChange={e => setTtsInput(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleSaveTts()}
              />
              <button type="button" onClick={() => setTtsShow(v => !v)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-on-surface transition-colors"
                aria-label="키 표시 전환">
                <span className="material-symbols-outlined text-base">{ttsShow ? 'visibility_off' : 'visibility'}</span>
              </button>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={handleSaveTts} disabled={!ttsInput || !ttsInput.trim()}
              className={`flex-1 h-10 sm:h-11 px-4 rounded-lg text-sm sm:text-label-md font-medium active:scale-95 transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2
                ${ttsSaved ? 'bg-green-500 text-white' : 'btn-orange disabled:opacity-40 disabled:cursor-not-allowed'}`}>
              <span className="material-symbols-outlined text-base">{ttsSaved ? 'check_circle' : 'save'}</span>
              {ttsSaved ? '저장되었습니다!' : '키 저장'}
            </button>
            {ttsApiKey && (
              <button onClick={() => { setTtsInput(''); onSaveTtsKey(''); }}
                className="h-10 sm:h-11 px-4 rounded-lg text-sm sm:text-label-md font-medium active:scale-95 transition-all flex items-center justify-center gap-1 text-error hover:bg-error-container/30 border border-error-container">
                <span className="material-symbols-outlined text-base">delete</span>
                삭제
              </button>
        ...
        {ttsApiKey && (
          <div className="mt-4 flex items-center gap-2 text-xs text-green-600 dark:text-green-400">
            <span className="material-symbols-outlined text-base">check_circle</span>
            Google TTS API 키가 설정되어 있습니다. Neural2 고품질 음성이 활성화됩니다.
          </div>
        )}
      </section>
    </div>
